...
class UserController extends AbstractActionController
{
    ...
    const ROUTE_CHANGEUSER  = 'zfcuser/changeuser';
    ...
    {
        // if the user isn't logged in, we can't change user
        if (!$this->zfcUserAuthentication()->hasIdentity()) {
            // redirect to the login redirect route
            return $this->redirect()->toRoute($this->getOptions()->getLoginRedirectRoute());
        }

        $form = $this->getChangeUserForm();
        $request = $this->getRequest();
        $request->getPost()->set('identity', $this->getUserService()->getAuthService()->getIdentity()->getEmail());

        $fm = $this->flashMessenger()->setNamespace('change-user')->getMessages();
        if (isset($fm[0])) {
            $status = $fm[0];
        } else {
            $status = null;
        ...
        $prg = $this->prg(static::ROUTE_CHANGEUSER);
        if ($prg instanceof Response) {
            return $prg;
        } elseif ($prg === false) {
            return array(
        ...
        if (!$change) {
            $this->flashMessenger()->setNamespace('change-user')->addMessage(false);
            return array(
                'status' => false,
                'changeUserForm' => $form,
            );
        ...
        $this->flashMessenger()->setNamespace('change-user')->addMessage(true);
        return $this->redirect()->toRoute(static::ROUTE_CHANGEUSER);
    }
     ...
     */
    public function getChangeUserForm()
    {
        if (!$this->changeUserForm) {
            $this->setChangeUserForm($this->getServiceLocator()->get('zfcuser_change_user_form'));
        }
     ...
     * Set changeUserForm.
     *
     * @param changeUserForm the value to set.
     */
    public function setChangeUserForm($changeUserForm)
    {
